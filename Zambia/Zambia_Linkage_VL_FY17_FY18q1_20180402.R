...
linkage_data2 <-linkage_data %>%
  ...
  select(psnu,psnuuid,facilityuid,facility,indicator,standardizeddisaggregate,numeratordenom,age,sex,fy2017q1,fy2017q2,fy2017q3,fy2017q4,fy2017apr,fy2018q1) %>% 
  group_by(psnu,psnuuid,facilityuid,facility,indicator,numeratordenom,age,sex,add=TRUE) %>% 
...
linkage_data2$HTS_TST_N <-rowSums(linkage_data2[,8:9], na.rm=TRUE)

#reorder columns
linkage_data2<-linkage_data2[c(1,2,3,4,5,6,7,13,8,9,10,12,11)]
...
totalN2 <- totalN %>% 
  ...
  select(psnu,psnuuid,facilityuid,facility,indicator,standardizeddisaggregate,numeratordenom,fy2017q1,fy2017q2,fy2017q3,fy2017q4,fy2017apr,fy2018q1) %>% 
  group_by(psnu,psnuuid,facilityuid,facility,indicator,standardizeddisaggregate,numeratordenom,add=TRUE) %>% 
  summarize_at(vars(starts_with("fy2")),funs(sum(.,na.rm=TRUE))) %>% 
  ungroup() %>% 
  unite(indicator_unite,indicator,numeratordenom,sep="_") %>% 
  gather(qtr, value, starts_with("fy2")) %>% 
...
totalN2 <- totalN2 %>% 
  ...
  select(psnu,psnuuid,facilityuid,facility,age,sex,qtr,HTS_TST_N,HTS_TST_NEG_N,HTS_TST_POS_N,TX_NEW_N,TX_PVLS_N,TX_PVLS_D)
